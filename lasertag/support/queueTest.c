...
{
	queue_index_t index = 0;
	queue_size_t count = queue_elementCount(q);

	printf("queue name: %s\n", q->name);
	while (count--) {
		printf("%lf\n", queue_readElementAt(q, index++));
	}
}
...
static bool compareChainOfSmallQueuesWithLargeQueue(void) {
  bool success = true;
...
static bool queue_pushPopTest(void) {
  bool testResult = true;
  bool tempResult = true;
...
bool queue_overwritePushTest(void) {
  ...
  for (uint16_t i = 0; i < OVERWRITE_PUSH_TEST_QUEUE_SIZE; i++) {
    // TODO remove me
    //printf("correct:%i, ours:%f\n", dataArray2[i], queue_readElementAt(&testQ, i)); 
